...
`agent-tuning-decision` is a portable Agent Skill for making better Agent tuning decisions in Codex and Claude Code. It helps the coding agent avoid jumping straight to fallback code, trace the root cause of an Agent behavior issue, compare past and current behavior, choose the right fix layer, and verify Prompt or orchestration changes with real historical turns when possible.
...
This skill makes the coding agent slow down at the right moment and answer:
...
The skill pushes the coding agent toward one recommended path first, backed by evidence and tradeoffs.
...
For Prompt and orchestration changes, the skill encourages the coding agent to evaluate changes against real past behavior.
...
The coding agent should recommend a small backtest set starting with recent true turns, define expected behavior before judging output, compare baseline and candidate tool calls, and report whether the Prompt change is better, worse, neutral, narrow-only, or inconclusive.
...
The coding agent should inspect orchestration first: whether the UI response path updated state, whether continuation was invoked, whether stale context was used, and whether the event sequence ended too early. Fallback copy should only be considered after the orchestration source is understood.

### Install

#### From This Repository

Clone the repo and copy the skill folder into your agent's skill directory.
...
For Claude Code as a user skill:

```bash
git clone https://github.com/linjiongxie/agent-tuning-decision-skill.git
mkdir -p "$HOME/.claude/skills"
cp -R agent-tuning-decision-skill/agent-tuning-decision "$HOME/.claude/skills/"
```

For Claude Code as a project skill, copy `agent-tuning-decision/` into your repository's `.claude/skills/` directory.

Restart Codex or Claude Code after installing so the skill metadata is loaded.
...
Claude Code does not need the Codex Skill Installer path. The same `agent-tuning-decision/SKILL.md` folder is enough when installed under `~/.claude/skills/` or a project's `.claude/skills/`.

### Explicit Invocation

You can force the skill by naming it. In Codex:
...
```text
Use the agent-tuning-decision Skill to trace why this Agent behavior appeared and recommend whether to tune the Prompt, orchestration, contract, or fallback layer.
```
...
The skill itself is only the `agent-tuning-decision/` folder. Root-level files are for sharing and documentation. `agents/openai.yaml` provides Codex/OpenAI metadata; Claude Code discovers the skill from `SKILL.md` and does not need a Claude-specific YAML file.
...
`agent-tuning-decision` 是一个用于 Agent 调优决策的可移植 Agent Skill，支持 Codex 和 Claude Code。它帮助编码 agent 避免一上来就补兜底代码，而是先把 Agent 行为问题的根因追到底，对比过去和现在的行为，选择正确的修复层级，并在可能时用真实历史 turn 验证 Prompt 或工作流编排改动。
...
这个 skill 会让编码 agent 在合适的位置慢下来，先回答：
...
这个 skill 会推动编码 agent 先给一个推荐路径，并用证据和取舍支撑它。
...
对于 Prompt 和工作流编排改动，这个 skill 会鼓励编码 agent 用真实历史行为来验证。
...
当这个 skill 用在非平凡的 Agent 调优问题上时，编码 agent 应该按这个形态回答：
...
编码 agent 应该先追踪这个选项来自当前 Agent 输出、Prompt 指令、后端 payload 默认值、replay 状态、前端 fallback，还是 UI 渲染。如果它是隐藏 fallback 代码合成出来的，推荐路径应该是删除或显式化这个来源，而不是调 Prompt 或继续加去重逻辑。
...
编码 agent 应该建议从最近真实 turn 开始构造一个小回测集，先写清楚期望行为，再比较 baseline 和 candidate 的工具调用，并判断 Prompt 改动是更好、更差、中性、只修了窄场景，还是因为样本或观测不足而无法判断。
...
编码 agent 应该先检查工作流编排：UI response 路径是否更新了状态、是否触发 continuation、是否用了过期上下文、事件序列是否过早结束。只有在编排来源被查明后，才考虑 fallback 文案。

### 安装

#### 从这个仓库安装

克隆仓库，并把 skill 文件夹复制到对应 agent 的 skills 目录。
...
Claude Code 项目级 skill：把 `agent-tuning-decision/` 复制到目标仓库的 `.claude/skills/` 目录。

安装后重启 Codex 或 Claude Code，让新的 skill metadata 生效。
...
Claude Code 不需要走 Codex Skill Installer 路径。只要把同一个 `agent-tuning-decision/SKILL.md` 文件夹安装到 `~/.claude/skills/` 或项目的 `.claude/skills/` 下即可。

### 显式调用

你可以通过点名强制使用这个 skill。Codex 中：
...
真正的 skill 只有 `agent-tuning-decision/` 文件夹。根目录文件用于共享和说明。`agents/openai.yaml` 是 Codex/OpenAI 侧元数据；Claude Code 会从 `SKILL.md` 发现 skill，不需要额外的 Claude YAML 文件。
